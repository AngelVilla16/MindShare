...
async function eliminarPost(idPost) {
    ...
    try {
        const res = await fetch("../../Controlador/PHP/eliminar_post.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
async function editarPost(idPost, encabezadoActual, cuerpoActual) {
    // For now, simple prompts. In a real app, a modal would be better.
    ...
    try {
        const res = await fetch("../../Controlador/PHP/editar_post.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
